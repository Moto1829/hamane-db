...
- [x] hnswlib 方式の並列挿入: ノード単位のロック (`Vec<Mutex<()>>` 相当) で
...
- [x] `HnswParams.build_threads: usize` (既定 = 物理コア数、1 で従来動作)
- [x] 並列構築は挿入順が非決定になるため、**決定性の扱いを明示する**:
      `build_threads == 1` のときのみ決定的 (deterministic_build テストは 1 固定)。
      セグメントの「同一入力 → 同一バイト列」保証は失われる旨を
      docs/design/storage.md の決定性の記述に追記
- [x] rayon は使わず std::thread + チャネル or スコープドスレッドで
