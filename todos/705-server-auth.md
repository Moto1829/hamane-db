...
- [x] 起動オプション `--api-key <KEY>` と環境変数 `HAMANE_API_KEY`
      (フラグ優先)。未指定なら認証なし (ローカル開発向け、起動時に警告)
- [x] ミドルウェアで全エンドポイントを保護:
      `Authorization: Bearer <key>` または `X-Api-Key: <key>` を受理
- [x] キー比較は定数時間 (タイミング攻撃対策)
- [x] 不一致・欠落は 401 (`{"error": "unauthorized"}`)
- [x] テスト: キーあり (401 / Bearer / X-Api-Key の 200) とキーなし (素通し)
