...
- **初期値は乱数直交行列** (Gram-Schmidt、seed 固定)。恒等行列から始めると
  軸に沿ったデータで `M = ŶᵀX` がほぼ対角になり極因子が恒等を返すため
  1 歩も動かない (恒等が局所最適)。設計文書 §1.1 に追記済み
- 極分解の Newton 反復は逆行列が取れないたびにリッジを 10 倍しながら最大 8 回
  粘る。逆行列は桁落ちを避けるため f64 で Gauss-Jordan (部分ピボット選択)
...
- opq 9 テスト + core 全 34 テスト green、clippy 0 (1.98 でも確認)
