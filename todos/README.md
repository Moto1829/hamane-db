...
4bit × m=64 が 8bit × m=32 より recall が高く (0.9863 vs 0.9838)、構築が
2.3 倍速い** (32.7 s vs 76.0 s)。代償は QPS (9402 → 6391、表引き回数が倍 +
ニブル展開)。m 据え置きの 4bit はコード半分で recall 0.907 (ef を上げても
改善しない = 律速は 1 段目)。
...
(16 エントリ LUT をシャッフル命令で一括評価。M12 の QPS 低下を取り戻す)。
