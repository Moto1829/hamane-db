...
レコード型はフォーマット変更が要るので別タスク。

要点:
- 一括更新は**カーソルで前へ進める**。「更新後も条件に一致し続ける」場合に
  毎回先頭から走査すると終わらないため (1602 の削除とはここが違う)
- `_ext_id` の変更は拒否する (文字列 ID の解決が壊れる)
- 実装中に **M16 の `ScanBuilder` が再エクスポートされていない**のを発見して修正
- Rust / HTTP (PATCH ×2) / CLI (`update-meta`) / Python (`update_meta`,
  `update_meta_by_filter`) に露出

## M18: collection のリネームと入れ替え (2026-09-13 完了)
...
WAL にタグ 5・6 を追加 (既存タグの意味は不変なので、新しいバイナリは
古い WAL を読める)。Rust / HTTP (`/rename`, `/admin/collections/swap`) /
CLI (`rename`, `swap`) / Python に露出。

## M19: メタデータ専用の WAL レコード (2026-09-13 完了)

| # | タスク | Depends |
|---|---|---|
| ✅ [1901](1901-wal-update-meta.md) | メタデータ更新の WAL 量を次元非依存にする | 1701, 1801 |

1701 の `update_meta` は内部で upsert に落としていたので、WAL にレコード全体が
載っていた。WAL タグ 7 (`UpdateMeta`) を追加してメタデータだけを書くようにした。

実測 (100 件更新の WAL 増分): dim=8 も dim=512 も **5,500 バイトで一致**
(次元非依存)。同じ更新を upsert でやると dim=512 で 210,700 バイトなので
**38 分の 1**。API と意味論は 1701 のままで、既存テストは無変更で通る。

## M20: ドキュメントの読みやすさ (2026-09-13 完了)

| # | タスク | Depends |
|---|---|---|
| ✅ [2001](2001-glossary.md) | 用語集と、専門用語の初出解説 | — |

仕様書は専門用語を説明なしで使っていた (セグメント 77 回 / WAL 43 / HNSW 43 /
memtable 29 / manifest 27)。ベクトル DB や LSM 系ストレージに馴染みが無いと
最初のページで詰まる。

用語集を 44 項目・5 カテゴリで新設し、各章の初出語からリンクした。
リンク切れは生成 HTML を突き合わせて検証済み (内部リンク 155 本すべて解決)。
