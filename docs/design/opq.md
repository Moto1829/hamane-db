...
- Status: v0.1 (2026-09-06、実装・計測済み)
...
train:
    best ← (誤差(I), I)                     # 回転なし = 素の PQ
    R₁ ← optimize(I);        best ← 誤差が小さければ更新
    if R₁ が I からほぼ改善していない:
        R₂ ← optimize(乱数直交行列); best ← 誤差が小さければ更新
    return best
最後に C は書き出し側 (write_pq / write_ivfpq) で通常の反復数で再学習する
```
...
**初期値は「両方試して実測で選ぶ」**。交互最適化は局所最適に落ちるため、
どちらの初期値が良いかはデータ依存で、片方に決め打つと悪化しうる:

| 初期値 | 得意 | 苦手 |
|---|---|---|
| 恒等行列 `I` | 自然な次元順に構造があるデータ (SIFT はサブベクトルが元から相関しており、位置分割が既に良い) | 軸に沿った独立分布。`M` がほぼ対角になり極因子が `I` を返すので**1 歩も動かない** (恒等が停留点) |
| 乱数直交行列 | 上記の停留点からの脱出 | 構造化データでは相関を壊すため出発点が悪すぎ、有限反復では回復しない |
| パラメトリック (§1.1.1) | 分散の偏り・次元間相関のあるデータ全般。乱数より良い出発点 | 共分散が縮退している場合 (固有値がほぼ 0) は情報が乏しい |

実測 (dim=32, m=8, ブロック相関データ): 素の PQ の MSE 1.109 に対し、
恒等初期値は 1.093 (改善)、乱数初期値は 12 反復かけても 16.1 (大幅悪化)。
一方、軸に沿った分散偏りデータでは恒等は動かず乱数初期値が勝つ。

したがって `train` は **恒等初期値 → (改善しなければ) パラメトリック初期値** の
順に試し、学習サンプル上の実測誤差が最小の候補を採る。さらに「回転なし」の誤差
とも比べ、勝てなければ恒等行列を返す。**OPQ を有効にして素の PQ より悪くなる
ことはない**。

### 1.1.1 パラメトリック初期値 (todos/1105)

恒等が停留点になるケースの救済には、乱数回転より**パラメトリック OPQ** の方が
良い出発点になる。学習サンプルの共分散 `Σ` を固有分解し (循環 Jacobi 法。
対称行列なので SVD は不要)、固有値 `λ` の**積がサブベクトル間で均等になるよう**
固有ベクトルを配る (固有値割り当て):

```
λ 降順に見て、その時点で「λ の積が最小」のサブベクトルへ入れる
R_param = 割り当て順に並べた固有ベクトル (行 = 固有ベクトル)
```

PQ の歪みは各サブベクトルの分散の幾何平均 `g_j` の和で近似でき、
`Π g_j` が回転で不変なので AM-GM より **`g_j` を均等にするのが最小化**になる。
回転後の次元が無相関になる (Σ が対角化される) のも PQ の独立量子化と相性が良い。
...
ただし**素の反復は実データで収束しない**。`M = ŶᵀX` は行数ぶんの和なので成分が
1e3〜1e4 になり、特異値の散らばりが大きいまま 50 反復しても直交にならず、
「極分解失敗 → 学習が 1 歩も進まない」となる (実際にこれで OPQ が乱数回転の
ままになり、SIFT の recall が PQ より悪化した)。Higham のスケーリングを入れる:
...
実測 (dim=128, m=32, n=200k, SIFT): フラッシュ全体で PQ 75-82 s → OPQ 111-116 s
(+35〜54%)。候補比較 (恒等初期値・乱数初期値・回転なし) を含む値。検索側の
オーバーヘッドは QPS 差が誤差範囲に収まる程度 (docs/benchmarks.md M11)。
