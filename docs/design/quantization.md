...
実装済みは **8 と 4** (§8)。6 のような 8 で割り切れない幅は詰め方が複雑なわりに
得が少ないので採らない。
...
## 8. 4-bit サブコード (M12, todos/1201〜1203)

`nbits = 4` は 1 サブベクトルを 16 セントロイドで表し、**2 サブコードを 1 バイト**に
詰める (偶数 `j` が下位ニブル)。1 行のコードは `ceil(m × nbits / 8)` バイト。

### 8.1 フォーマット

vectors_pq.bin / ivfpq.bin のヘッダには**最初から `nbits` と `ksub` が入っている**
ので、フォーマットの変更は不要 (自己記述的)。ロード時に
`nbits ∈ {4, 8}` と `ksub == 2^nbits`、コード領域が
`count × code_bytes(m, nbits)` であることを検証する。8bit で書いた既存セグメントは
そのまま読める。

### 8.2 パラメータの決め方

コード長 (= メモリ) を固定して比べるなら **`nbits=4` は `m` を 2 倍**にする。
サブベクトルが細かくなるぶん、セントロイド数が減る不利を相殺できる。

| 構成 | コード長 (dim=128) | 位置づけ |
|---|---|---|
| m=32, nbits=8 | 32 B | 既定 |
| m=64, nbits=4 | 32 B | 同じメモリ。分割が細かい |
| m=32, nbits=4 | 16 B | メモリ半分。1 段目は粗くなる |

### 8.3 再ランク候補数

4-bit は 1 段目の順位付けが粗いので、`k × 4` では真の近傍が候補に入らないことが
ある。**nbits=4 のときは `k × 8`** を取ってから f32 で並べ直す
(`rerank_factor`)。dim=16 の乱数データでの実測 (統合テスト):

| 構成 | k×4 | k×8 |
|---|---|---|
| m=4, nbits=4 (2 B/行) | 0.685 | 0.822 |
| m=8, nbits=4 (4 B/行) | 0.952 | 1.000 |
| m=4, nbits=8 (4 B/行) | 0.985 | — |

### 8.4 StoreOptions

`StoreOptions.pq_nbits: u8` (既定 8)。`Quantization::Pq` 以外との併用と、
4/8 以外の値は open 時に `InvalidConfig`。OPQ (M11) の回転学習も同じ nbits で
行う (回転は実際に使うコード幅に対して最適化する)。

### 8.5 バイト単位の合成 LUT (todos/1204)

素直に実装すると 4bit は表引きが `m` 回 (8bit の 2 倍) に増え、さらにニブル展開が
入るため QPS が 3 割落ちる。4bit は**1 バイトに 2 サブコードが入る**ので、
バイト値をそのまま引ける合成表を作ればよい:

```
fused[i][b] = sub[2i][b & 0x0f] + sub[2i+1][b >> 4]     (i < m/2, b は 0..256)
distance_key = Σ_i fused[i][code[i]]
```

- 表引きは `m/2` 回 = 同じコード長の 8bit と同じループ回数になり、展開も消える
- 構築コストは `(m/2) × 256` 回の加算で、クエリごとに 1 回だけ (行数に比べ誤差)
- 表のサイズは `(m/2) × 256 × 4 B` = m=64 で 32 KB。8bit の `m × 256 × 4 B`
  (m=32 で 32 KB) と同じ
- `m` が奇数のときは末尾 1 サブベクトルだけ 16 エントリ表で持つ
- 実測: SIFT 200k・m=64 で QPS 6391 → 9492 (8bit の 9402 と同等)。recall は不変
