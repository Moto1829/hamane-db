...
- 4 のセグメント検索は Database 全体で共有する常駐スレッドプールで並列化する
  (503 で並列化、801 でプール化)。並列度は `StoreOptions::search_threads`
  (0 = 自動 = 論理コア数、1 = 逐次)。呼び出しスレッドも 1 セグメントを担当し、
  worker はプール並列度 − 1 本を初回検索時に遅延起動する。
  memtable 検索は呼び出しスレッドで逐次
