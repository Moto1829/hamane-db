...
| m=64, 4bit | 32 B | **0.9863** | **0.9932** | 6391 | **32.7 s** | 136 MB |
| m=32, 4bit | 16 B | 0.9073 | 0.9081 | 8835 | 32.4 s | 133 MB |

読み取り:

- **同じコード長なら 4bit × m 2 倍が有利**。recall は 8bit をわずかに上回り
  (0.9863 vs 0.9838)、**構築は 2.3 倍速い** (32.7 s vs 76.0 s)。k-means の
  セントロイドが 256 → 16 に減るコスト削減 (16x) が、m を 2 倍にする増加 (2x) を
  大きく上回るため
- **QPS は 3 割ほど落ちる** (9402 → 6391)。1 行あたりの表引きが 32 回から 64 回に
  増え、さらにニブル展開が入るため。LUT 自体は小さくなる (32×256 → 64×16
  エントリ) がループ回数が効く。**PQ4 の SIMD fast-scan (16 エントリの LUT を
  シャッフル命令で一括評価) が将来の最適化余地**
...
**結論**: PQ を使うなら `pq_nbits: 4` + `m` 2 倍が既定として妥当 (同メモリ・
同 recall・構築 2.3 倍速)。ただし QPS を最優先するなら 8bit のままがよい。
メモリ最優先なら m 据え置きの 4bit で recall を 7 ポイント譲る。
