...
- 日付: 2026-09-11
- 環境: Apple Silicon (aarch64) / macOS / rustc 1.93.0 / release
- 構成: 20 万件 × dim=64、**単一セグメント**、500 クエリ (k=10)

| 方式 | 所要 |
|---|---|
| 1 件ずつ `search` | 44.0 ms |
| `search_batch` | **5.1 ms** (8.7x) |

結果は完全に一致する (並列化は順序・内容に影響しない)。

読み取り: セグメントが 1 個なので**従来のセグメント間並列は全く効かない**構成。
バッチはクエリ間で並列化するため、この条件でこそ効く。加えて
`LiveView` の取得がクエリ数ぶん繰り返されなくなる。
セグメントが多い DB では既存の並列化と食い合うので、倍率はこれより小さくなる。

## M19: メタデータ更新の WAL 量 (todo 1901)

- 日付: 2026-09-13 / 環境: Apple Silicon (aarch64) / release
- 100 件のメタデータ更新で、フラッシュ直後からの WAL 増分を計測

| 経路 | dim=8 | dim=512 |
|---|---|---|
| `update_meta` (メタデータ専用レコード) | 5,500 B | **5,500 B** |
| `upsert` で同じ更新 (M17 までの内部実装) | — | 210,700 B |

**次元に依存しません** (ベクトルを WAL に載せないため)。dim=512 では
38 分の 1。WAL は fsync を伴う直列パスなので、一括タグ付けのような
運用操作のスループットに直結します。

memtable 側のコストは変わりません (セグメント上の行を上書きするには
ベクトルごと memtable に載せる必要がある)。減るのは WAL の書き込み量だけです。
